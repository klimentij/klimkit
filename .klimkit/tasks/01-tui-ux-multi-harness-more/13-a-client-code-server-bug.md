...
Live evidence before the fix showed `Workstation-8.local` reporting with an empty `machine_dns`, causing Mac workspaces to have empty backend `code_server_url` values and letting the browser fall back to dev-vm URLs.
...
- [x] Let the central Switchboard server resolve missing client DNS from Tailscale peer data, including fuzzy `Workstation-8.local` to `workstation-23.example-tailnet.ts.net` matching.
...
- [x] Apply locally and verify the live Mac manual tab iframe URL points at `workstation-23.example-tailnet.ts.net`, not `dev-vm.example-tailnet.ts.net`.
...
- Live API proof: `Workstation-8.local` now resolves to `workstation-23.example-tailnet.ts.net`, and Mac workspace `code_server_url` values use that hostname.
- Browser QA proof: a stale manual Mac tab saved with an dev-vm URL was corrected to `https://workstation-23.example-tailnet.ts.net/?folder=%2FUsers%2Foperator%2Fcoding-ops` for both `Open directly` and iframe `src`.
