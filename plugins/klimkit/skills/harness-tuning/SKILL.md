---
name: harness-tuning
description: Tune the human's shared Codex home-level harness pack safely through the Klimkit repo, not by editing generated files in ~/.codex directly.
---

# Harness Tuning

Use this skill when the human asks to change shared Codex behavior, subagents, skills, hooks, model defaults, or other home-level harness files.

Current projected artifact workflow: `solo`; operator folder: `<operator>`; writable artifact root: `.klimkit`.
...
- Push committed pack changes to `main` when the human wants the update everywhere. Machines with autosync enabled poll `origin/main` every 5 seconds by default, fast-forward, apply projections, restart managed services, and send Telegram summaries when configured.
